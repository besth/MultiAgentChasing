...
class WolfPositionTransition():
    def __init__(self, numOneAgentState, positionIndex, checkBoundaryAndAdjust, wolfSpeed, chasingSubtlety):
        self.numOneAgentState = numOneAgentState
        self.positionIndex = positionIndex
        self.checkBoundaryAndAdjust = checkBoundaryAndAdjust
        self.wolfSpeed = wolfSpeed
        self.chasingSubtlety = chasingSubtlety
    def __call__(self, oldAllAgentState, wolfId, sheepId):
        ...
        heatSeekingDirectionPolar = ag.transiteCartesianToPolar(oldSheepPosition - oldWolfPosition)
        wolfVelocityDirectionPolar = np.random.vonmises(heatSeekingDirectionPolar, self.chasingSubtlety) 
        wolfDirection = ag.transitePolarToCartesian(wolfVelocityDirectionPolar)
        #__import__('ipdb').set_trace()
        newWolfVelocity = self.wolfSpeed * wolfDirection
