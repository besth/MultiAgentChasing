...
def runTrial(modelDf):
    ...
    levelNames = ['sheepXPosInit', 'sheepYPosInit', 'wolfXPosInit', 'wolfYPosInit', 'cInit', 'cBase', 'numSimulations',
                  'trialIndex', 'maxRunningSteps', 'sheepPolicyName', 'render', 'killzoneRadius', 'aliveBonus', 'deathPenalty',
                  'rolloutHeuristicWeight']
    levelValues = [modelDf.index.get_level_values(levelName)[0] for levelName in levelNames]
    sheepXPosInit, sheepYPosInit, wolfXPosInit, wolfYPosInit, cInit, cBase, numSimulations, trialIndex, maxRunningSteps, sheepPolicyName, render, killzoneRadius, aliveBonus, deathPenalty, rolloutHeuristicWeight = levelValues
    ...
    if render:
        ...
        if len(frames) != 0:
            ...
            skvideo.io.vwrite("./videos/videoTrial{}.mp4".format(trialIndex), frames)
...
def main():
    ...
    cInit = [1]
    cBase = [100]
    numSimulations = [5]#[50]#, 250, 500]#, 1000, 1500]
    trialIndex = [1, 2, 3, 4]
    maxRunningSteps = [5]
    ...
    levelValues = [sheepXPosInit, sheepYPosInit, wolfXPosInit, wolfYPosInit, cInit, cBase, numSimulations, trialIndex, maxRunningSteps, sheepPolicyName, render, killzoneRadius, aliveBonus, deathPenalty, rolloutHeuristicWeight]
    levelNames = ['sheepXPosInit', 'sheepYPosInit', 'wolfXPosInit', 'wolfYPosInit', 'cInit', 'cBase', 'numSimulations',
                      'trialIndex', 'maxRunningSteps', 'sheepPolicyName', 'render', 'killzoneRadius', 'aliveBonus', 'deathPenalty',
                      'rolloutHeuristicWeight']
    ...
    modelResultDf = toSplitFrame.groupby(levelNames).apply(runTrial)
    measurementDf = modelResultDf.groupby(levelNames).apply(measurementFunction)
    print("measurementDf: ")
    print(measurementDf)

    levelNames.remove('trialIndex')
    dataStatisticsDf = measurementDf.groupby(levelNames).distance.agg(['mean', 'std'])
    print(dataStatisticsDf.index)
    ...
    #
    # for (key, dataDf) in dataStatisticsDf.groupby(levelNames):
    #     axForDraw = fig.add_subplot(plotRowNum, plotColNum, plotCounter)
    #     drawPerformanceline(dataDf, axForDraw, variableParameter1)
    #     plotCounter += 1
    #
    # plt.legend(loc = 'best')
    # plt.xlim((0, 10))
